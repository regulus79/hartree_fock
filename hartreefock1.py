...
def energy(coeffs, num_occupied_orbitals, nuclei, S, T, V, W):
	...
	for orbital_coeffs in occupied_orbitals.T:
		# Electron 1
	...
	for orbital_coeffs1 in occupied_orbitals.T:
		for orbital_coeffs2 in occupied_orbitals.T:
			two_electron_energy += 4*orbital_coeffs2 @ (orbital_coeffs1 @ W @ orbital_coeffs1) @ orbital_coeffs2 #/ (orbital_coeffs1 @ S @ orbital_coeffs1) / (orbital_coeffs2 @ S @ orbital_coeffs2)
			two_electron_energy += -2*orbital_coeffs2 @ (orbital_coeffs1 @ W @ orbital_coeffs2) @ orbital_coeffs1 #/ (orbital_coeffs1 @ S @ orbital_coeffs1) / (orbital_coeffs2 @ S @ orbital_coeffs2)
...
def fock_operator(coeffs, num_occupied_orbitals, nuclei, S, T, V, W):
	...
	for orbital_coeffs1 in occupied_orbitals.T:
		two_electron_operators += 2 * (orbital_coeffs1 @ W @ orbital_coeffs1) #/ (orbital_coeffs1 @ S @ orbital_coeffs1)
		two_electron_operators += -(orbital_coeffs1 @ W.transpose((0,3,2,1)) @ orbital_coeffs1) #/ (orbital_coeffs1 @ S @ orbital_coeffs1)
	# DO NOT DIVIDE BY 2 WHEN CONSTRUCTING THE FOCK OPERATOR
...
print("Max/min/avg S:", np.max(S), np.min(S), np.average(S))
print("Max/min/avg T:", np.max(T), np.min(T), np.average(T))
print("Max/min/avg V:", np.max(V), np.min(V), np.average(V))
print("Max/min/avg W:", np.max(W), np.min(W), np.average(W))
...
# OH! These are not orthonormal, so energy calc wrong. That makes more sense
initial_coeffs = orthonormal_basis_coeffs @ np.identity(len(basisSet))
...
def scf_iteration(S, T, V, W, NR, initial_coeffs):
	#print(initial_coeffs.T @ S @ initial_coeffs)
	...
	new_coeffs_ortho = eigenvecs[:,np.argsort(eigenvals)]
	new_coeffs = (orthonormal_basis_coeffs @ new_coeffs_ortho)
	new_coeffs2 = eigenvecs2[:,np.argsort(eigenvals2)]
	#print("new_coeffs", new_coeffs.T @ S @ new_coeffs)
	#print("new_coeffs2", new_coeffs2.T @ S @ new_coeffs2)
